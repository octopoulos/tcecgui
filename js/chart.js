...
function generateTicks$1(generationOptions, dataRange) {
    let max = dataRange.max,
        ...
        ticks = generateTicks(
            ...
            {
                max: (max >= 1)? Log10(max): (max <= -1)? -Log10(max): 0,
                min: (min >= 1)? Log10(min): (min <= -1)? -Log10(min): 0,
            });
    ...
    return ticks.map(tick => {
        ...
        for (let i = 2; i >= 0; i --) {
            ...
            if (Abs(pow2 / (first + pow2) - 0.5) > 0.03)
                return pow;
            pow = pow2;
        }
        return pow;
...
function getMin(options) {
    return Undefined(options.time.min, options.ticks.min);
}

function getMax(options) {
    return Undefined(options.time.max, options.ticks.max);
}
...
function buildLookupTable(timestamps, min, max, distribution) {
    ...
    for (i = 0, ilen = items.length; i < ilen; ++i) {
        ...
        if (prev === undefined || next === undefined || Round((next + prev) / 2) !== curr) {
...
function determineUnitForAutoTicks(minUnit, min, max, capacity) {
    ...
    for (i = UNITS.indexOf(minUnit); i < ilen - 1; ++i) {
        ...
        if (interval.common && Ceil((max - min) / (factor * interval.size)) <= capacity) {
            return UNITS[i];
        }
    }
