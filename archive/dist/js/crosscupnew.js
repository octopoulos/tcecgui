...
function pgn2crosstable(pgn, eventtable=false) {
    ...
    // COMMENTED!!
    console.log ("XXX: gamesReqd:" + gamesReqd);
    pgn = shlib.returnPGN(pgn, false, gamesReqd);
...
function get_score_and_games(crosstable, engine, ignore_engines=[]) {
    ...
    {
       engine_table.Mlead = 0;
       engine_table.MWinner = 0;
    }
}
    ...
    const compare_function = function (engine1_name, engine2_name) {
      try {
          ...
          if (engine1_data.Strikes >= dq_strikes && engine2_data.Strikes < dq_strikes) {
            ...
            console.log ("Failed for engine1_name:" + engine1_name + " ,engine2_name:" + engine2_name);
            }
      };
...
function eliminateEngines(original_crosstable, max_strikes=10) {
...
cup = retArg(argv.cup, cup);

let seasonFileNameFull = pgnPath + seasonFileName;
const pgn = fs.readFileSync(seasonFileNameFull, "utf-8");
let crossName = filename + '_' + 'Crosstable.cjson';

if (cup)
{
   crossName = filename + '_' + 'Eventcrosstable.cjson';
   eventtable = true;
}

if (argv.outfile != undefined)
{
   crossName = pgnPath + '/' + argv.outfile + '_' + 'Eventcrosstable.cjson';
}
...
let output_json = pgn2crosstable(pgn, eventtable);
console.log ("Outfile is:" + crossName);
if (cup)
{
